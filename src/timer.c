#include "timer.h" // 1ミリ秒あたりにCPUのTSCが進むカウント数（このファイル内のみで保持） 
static volatile uint64_t tsc_freq_per_ms = 0; 
...
void init_sleep_timer(void) { 
    ...
    while (1) { 
        ...
        uint32_t diff = (current_pm >= start_pm) ? (current_pm - start_pm) : ((0x1000000U + current_pm) - start_pm); 
